...
from model.fourier_data.augmentations import augmentations_all, augmentations_H, augmentations_L
from model.fourier_data.prepare_data import get_data_paths_list
...
def augmix(img, alpha, mixture_width, level, im_size, mixture_depth):

    aug_list = augmentations_L
    ws = np.float32(np.random.dirichlet([alpha] * mixture_width))
    m = np.float32(np.random.beta(alpha, alpha))
    mixed_image = torch.zeros_like(preprocess(img))
    ...
    for i in range(mixture_width):
        ...
        for d in range(depth):
            op = np.random.choice(aug_list)
            aug_img = op(aug_img, level, im_size)
        mixed_image += ws[i] * preprocess(aug_img)
    return m * preprocess(img) + (1-m) * mixed_image
